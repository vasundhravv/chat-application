...
function SidebarChat({ dataList, title, path, fetchList }) {
  ...
  useEffect(() => {
    ...
    if (fetchList) {
      a = function () {
        if (
          parseInt(sidebarChatContainer.current.scrollTop) ===
          sidebarChatContainer.current.scrollHeight -
            sidebarChatContainer.current.offsetHeight
        ) {
  ...
  const images = [
    ...
    `https://api.dicebear.com/8.x/adventurer/svg?seed=Lola`
  ];
      ...
      dataList.forEach((data) => {
        if (data) {
          ...
          if (
            title === "Rooms" ||
            title === "Search Result" ||
            (title === "Chats" && roomsData[data.id].lastMessage) ||
                  ...
                  <div className="sidebar__chat--info">
                    <h2
                      ...
                      style={{
                        ...
                        marginBottom:
                          lastMessage?.message || lastMessage ? 8 : 0,
                      }}
                    ></h2>
                    <p
                      style={{
